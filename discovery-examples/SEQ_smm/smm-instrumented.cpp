...
public:
  ...
  void multiply(sparse_matrix * b) {
    ...
    dfsan_begin_marking(LOOP1_RUN);
    // Mark whether A and B are sparse or dense
    dfsan_begin_marking(matrix_type);
    for (apos = 0; apos < len;) {
    ...
    dfsan_end_marking(matrix_type);
    dfsan_end_marking(LOOP1_RUN);
