...
Guesser::Guesser() //Buffer *buffer)
	...
	: Thread(), tbb::filter(serial_in_order)
{
	n = 0;
	Memblock *p_block = new Memblock;
  fprintf (stderr, "Called a constructor\n");
}
...
void Guesser::run()
{
  // VJ: We will need to put this somewhere else
  if (!m_resume) {
    try {
	  init();
...
/*
Memblock * Guesser::operator()(tbb::flow_control &fc)  {
//void * Guesser::operator()(tbb::flow_control &fc)  {
	  if (abortFlag()) {
		  fc.stop();
		  return NULL;
	  } else {
	    ...
	    if (watch.elapsed() > 2000) {
	      if (Attack::status() == Attack::STATUS_RUNNING) {
		    std::cout << "Rate: " << 1000 * (double)n/watch.elapsed() << " phrases / second. ";
		    std::cout << "Phrase: ";
		    for (uint32_t i = 0; i < p_block->length; i++) {
		      std::cout << (char)p_block->data[i];
		    }
		    std::cout << std::endl;
	      }
	      watch.start();
