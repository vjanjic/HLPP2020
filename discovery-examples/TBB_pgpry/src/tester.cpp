...
// Constructor
Tester::Tester(const Key &key)
	: Thread(), m_key(key), m_ivec(NULL), m_keydata(NULL),
	  m_in(NULL), m_out(NULL), filter(parallel)
{
	init();
}
...
void * Tester::operator() (void *input)
//void* Tester::operator() (void *input) const
{
	/* VJ: As per usual, move somwhere else
	try {
		init();
	} catch (const std::string &str) {
		Attack::error(str);
	...
	Memblock *in = (Memblock *) input;
	if (check(*in))
		Attack::phraseFound(*in);
