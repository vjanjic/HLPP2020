...
{
	m_prefixes = prefixes;
	prefixBlocks = (m_prefixes.empty() ? NULL : new Memblock[m_prefixes.size()]);
...
{
	m_suffixes = suffixes;
	suffixBlocks = (m_suffixes.empty() ? NULL : new Memblock[m_suffixes.size()]);
...
//Memblock * PrefixSuffixFilter::operator()(Memblock *input) const
void * PrefixSuffixFilter::operator()(void *input)
{
	Memblock buffer, buffer2;

	if (m_suffixes.empty()) {
		for (uint32_t i = 0; i < m_prefixes.size(); i++) {
			buffer = *((Memblock *)input);
			buffer += prefixBlocks[i];
			return (void *)(&buffer);
		}
	} else if (m_prefixes.empty()) {
