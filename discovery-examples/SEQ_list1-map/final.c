...
/* For the simple case. */
/* Very simple (sequential) map implementation. It's a straight copy of
   traverse. */
void Map(callback f, node* head)
{
  node* cursor = head;
  while(cursor != NULL)
    {
...
int main(int argc, char *argv[])
{
  ...
  /* Replaced call to traverse/2 with simple map (sequential, copy of traverse. */
  Map(inc, head);
 ...
 * Process:
 *
 * 1. Determine properties of custom data structure.
 *   - Type of list
 ...
 *       - Assignment only
 *   - Two calls to traverse, inspect function
 *     - traverse takes a node and a function,
 *     - function is applied to each element in the node
 *     - neither traverse nor the function does not change the spine
 *     - Conclude that traverse is a map operation (that can be parallelised)
 *
 */
