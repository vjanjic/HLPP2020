...
int32_t Attack::run(const Key &key, const Options &options, ConfReader *reader)
{
	...
	for (uint32_t i = 0; i < ctx->m_guessers.size(); i++) {
		...
		std::cout << "Gustav Krklec" << std::endl;
		assert(ctx->m_guessers[i]->m_running == false);
		ctx->m_guessers[i]->m_running = true;
		pthread_create(&ctx->m_guessers[i]->m_pth,
			NULL,
			&ctx->m_guessers[i]->main,
			&ctx->m_guessers[i]); // was `this`
		// NB `ctx->m_guessers[i]->` parts added in to replace make it link up
	}
