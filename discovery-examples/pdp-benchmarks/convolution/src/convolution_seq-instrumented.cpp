...
unsigned short * process_image(task_t task) {
  ...
  __begin_loop(LOOP111);
  for(int x = 0; x < dim; x++) {
    __begin_iteration(LOOP111);
    __begin_loop(LOOP112);
    for(int y = 0; y < dim; y++) {
      __begin_iteration(LOOP112);
      ...
      __begin_loop(LOOP119);
      for(int j = left; j <= right; j++) {
        __begin_iteration(LOOP119);
      ...
      __end_iteration(LOOP112);
    }
    __end_loop(LOOP112);
    __end_iteration(LOOP111);
  }
  __end_loop(LOOP111);
...
int main(int argc, char * argv[]) {
  ...
  dim = 4 ; //1024 ; // atoi(argv[1]);
  nr_images = 4 ; // 20 ; // atoi(argv[2]);
