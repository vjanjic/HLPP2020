...
int bs_thread(void *tid_ptr) {
    ...
    __begin_loop(LOOP285);
    for (j=0; j<NUM_RUNS; j++) {
      __begin_iteration(LOOP285);
#ifdef ENABLE_OPENMP
#pragma omp parallel for private(i, price, priceDelta)
        for (i=0; i<numOptions; i++) {
#else  //ENABLE_OPENMP
          __begin_loop(LOOP290);
          for (i=start; i<end; i++) {
            __begin_iteration(LOOP290);
#endif //ENABLE_OPENMP
            /* Calling main function to calculate option value based on 
             * Black & Scholes's equation.
             */
...
#endif
        __end_iteration(LOOP290);
        }
        __end_loop(LOOP290);
        __end_iteration(LOOP285);
    }
    __end_loop(LOOP285);
...
#define __PARSEC_XSTRING(x) __PARSEC_STRING(x)
   ...
   if (argc != 4)
    ...
    __begin_loop(LOOP373);
    for ( loopnum = 0; loopnum < numOptions; ++ loopnum )
    {
        __begin_iteration(LOOP373);
        rv = fscanf(file, "%f %f %f %f %f %f %c %f %f", &data[loopnum].s, &data[loopnum].strike, &data[loopnum].r, &data[loopnum].divq, &data[loopnum].v, &data[loopnum].t, &data[loopnum].OptionType, &data[loopnum].divs, &data[loopnum].DGrefval);
        if(rv != 9) {
          printf("ERROR: Unable to read from file `%s'.\n", inputFile);
          fclose(file);
          __end_iteration(LOOP373);
          __end_loop(LOOP373);
          exit(1);
        }
        __end_iteration(LOOP373);
