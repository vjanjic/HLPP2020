 ...
 Environment::Environment(){
	...
	for(int j=0; j<numDevices; j++){
        ...
        if((b & CL_TRUE) && (status == CL_SUCCESS)){
            //std::cout<< b<< " vs "<<CL_TRUE<< std::endl;
            //std::cout << "this device is available "<< deviceIds[j]<<std::endl;
            ...
            //std::cout<<"max application number for "<< clDevice->deviceId << clDevice->maxNum <<std::endl;
