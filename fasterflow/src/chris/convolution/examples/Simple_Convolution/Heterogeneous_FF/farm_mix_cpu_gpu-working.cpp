...
cl_uint *read_image(const char *fileName, png_uint_32 height) {
  ...
  printf ("about to read from image %s\n", fileName);
  ...
  {
    FILE *foutp = fopen ("out", "w");
    for (uint x=0; x<height*width; x++)
      fprintf (foutp, "%lu ", ((cl_uint *)raw_data)[x]);
    fclose(foutp);
    exit(1);
  }
  
  return (cl_uint *)row_pointers;
    ...
    if (task==NULL) return NULL;
      ...
      for(int x = 0; x < height; x++)
        for(int y = 0; y < width; y++) {
          ...
          right   = ((x + vstep) >= width) ? width - 1 : (x + vstep); 
          top     = (y           <  hstep) ? 0         : (y - hstep);
          bottom  = ((y + hstep) >= height)? height - 1: (y + hstep); 
          sumFX = 0;
...
int main(int argc, char * argv[]) {
  if (argc<2) 
    std::cerr << "use: " << argv[0] << " <noCPUworkers> <noGPUWorkers>\n";
  nworkers = atoi(argv[1]);
  gnworkers = atoi(argv[2]);
  //ff_farm<> farm;
  //Collector C;
