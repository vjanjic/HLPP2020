...
__kernel void simpleConvolution(__global  ushort  * output,
                                ...
                                __global  float  * mask,
                                const     uint  inputDimension,
                                const     uint  maskDimension)
{
    uint tid   = get_global_id(0);
    
    uint width  = inputDimension;
    uint height = inputDimension;
    
    uint x      = tid%width;
    uint y      = tid/width;
    
    uint maskWidth  = maskDimension;
    uint maskHeight = maskDimension;
