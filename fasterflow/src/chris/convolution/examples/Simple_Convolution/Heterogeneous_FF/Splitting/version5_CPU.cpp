...
public:
  ...
  int svc_init(){
    map_farm.add_collector(NULL);
    std::vector<ff_node*> mw;
    for(int i=0; i<mapworkers; i++)
      mw.push_back(new CPU_MAP_Stage);
    map_farm.add_workers(mw);
    return 0;
  }
  ...
  void * svc(void * task ) {
    if (task==NULL) return NULL;
      ...
      void *chunks = (void *) malloc (sizeof(point_type)*chunk_size);
      uint chunk_size = height * width / man_nworkers;
      uint chunk_pos = 0;
      uint curr_x = 0, curr_y = 0;
      
      map_farm.run_then_freeze();

      for (i=0; i<man_nworkers; i++) {
        chunk_pos = 0;
        ((map_task_t)chunks[i]).minx = curr_x;
        ((map_task_t)chunks[i]).miny = curr_y;
        ((map_task_t)chunks[i]).maxx = curr_x + chunk_size / width + 1;
        ((map_task_t)chunks[i]).maxy = (curr_y + chunk_size % width ) % height;
        ((map_task_t)chunks[i]).output = t->output;
        curr_x = chunks[i].maxx;
        curr_y = chunks[i].maxy;
        map_farm.offload(&(chunks[i]));
      }
      
      map_farm.offload((void *)FF_EOS);
      map_farm.wait_freezing();
...
class CPU_MAP_Stage: public ff_node {
    ...
    if (task==NULL) return NULL;
      ...
      for (x=t->minx; x<t->maxx; x++) {
        for (y=t->miny; y<t->maxy; y++) {
          ...
          sumFX += 0.5f;
          t->output[x*width+y] = (ushort) sumFX; //int(sumFX);
        }
      }
    }
    return GO_ON;
...
int main(int argc, char * argv[]) {
  ...
  // map_farm.push_back(new GPU_Stage);
  std::vector<ff_node*> mw;
  for(int i=0; i<mapworkers; i++)
	mw.push_back(new CPU_MAP_Stage);
