...
int main(int argc, char * argv[]) {
  ...
  if (argc<5) 
    std::cerr << "use: " << argv[0] << " <noCPUworkers farm1> <noCPUworkers farm2> <noCPUworkers farm3> <noGPUWorkers> <imageSize> <nrImages>\n";
  nworkers = atoi(argv[1]);
  nworkers2 = atoi(argv[2]);
  nworkers3 = atoi(argv[3]);
  gnworkers = atoi(argv[4]);
  height = atoi(argv[5]);
  NIMGS = atoi(argv[6]);
  width = height;
  ...
  for(int i=0; i<nworkers; ++i)
  {
       ...
       ff_farm<> * stage1 = new ff_farm<>;
       stage1->add_collector(NULL);
       std::vector<ff_node*> w2;
       for(int i=0; i<nworkers3; ++i)
       {  
            w2.push_back(new ff_pipe_first_stage);
       }
       stage1->add_workers(w2);
